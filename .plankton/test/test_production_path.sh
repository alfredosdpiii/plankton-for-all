...
# (subprocess delegation) using a mock `pi` binary.
#
# Verifies that when HOOK_SKIP_SUBPROCESS is NOT set, the hook delegates
# to a pi subprocess and verifies results afterward.
#
# Usage:
#   bash .plankton/test/test_production_path.sh
#   # or from project root:
#   .plankton/test/test_production_path.sh
...
# --- Helper: create a fake PLANKTON_PROJECT_DIR with config ---
setup_project_dir() {
  local pd="$1"
  local config_content="$2"
  mkdir -p "${pd}/.plankton/hooks"
  printf '%s\n' "${config_content}" >"${pd}/.plankton/config.json"
  # Create subprocess settings file (project-local)
  cat >"${pd}/.plankton/subprocess-settings.json" <<'SETTINGS_EOF'
{
...
# Mock pi: accepts any args, exits 0, does NOT modify files
cat >"${test1_dir}/bin/pi" <<'MOCK_EOF'
#!/bin/bash
# Mock pi that does nothing
exit 0
MOCK_EOF
chmod +x "${test1_dir}/bin/pi"
...
# Run the hook with mock pi on PATH
test1_stdout=""
test1_stderr=""
test1_exit=0
...
echo "${test1_json}" \
  | PATH="${test1_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test1_dir}/project" \
...
# Mock pi: finds file path from last arg, replaces with clean content
cat >"${test2_dir}/bin/pi" <<'MOCK_EOF'
#!/bin/bash
# Mock pi that fixes the file by replacing its content
...
test2_stderr=$(echo "${test2_json}" \
  | PATH="${test2_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test2_dir}/project" \
...
# Build a PATH with symlinks to all needed tools EXCEPT pi.
# This is necessary because pi and the linters may share a directory
...
echo "${test3_json}" \
  | PATH="${test3_bin}" \
    ...
    PLANKTON_PROJECT_DIR="${test3_dir}/project" \
...
# spawn_fix_subprocess returns 0 when pi is not found (graceful skip).
...
assert "test3_not_found" "echo '${test3_stderr}' | grep -q 'pi binary not found'" \
  "stderr contains 'pi binary not found'" \
  "stderr missing 'pi binary not found'"
...
# Mock pi: writes a marker file if called (should NOT be called)
test4_marker="${test4_dir}/pi_was_called"
cat >"${test4_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
# Mock pi that creates a marker file to prove it was called
touch "${test4_marker}"
exit 0
MOCK_EOF
chmod +x "${test4_dir}/bin/pi"
...
echo "${test4_json}" \
  | PATH="${test4_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test4_dir}/project" \
...
assert "test4_no_call" "[[ ! -f '${test4_marker}' ]]" \
  "mock pi was NOT called" \
  "mock pi was called (marker file exists)"
