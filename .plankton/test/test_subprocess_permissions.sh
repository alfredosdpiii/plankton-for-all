...
#
# Usage: bash .plankton/test/test_subprocess_permissions.sh
...
# Mock pi that logs all args to a file
test1a_args="${test1a_dir}/pi_args.txt"
cat >"${test1a_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${test1a_args}"
exit 0
MOCK_EOF
chmod +x "${test1a_dir}/bin/pi"
...
echo "${test1a_json}" \
  | PATH="${test1a_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test1a_dir}/project" \
...
# Mock pi that logs its environment to a file
test1d_dir="${tmp_dir}/test1d"
mkdir -p "${test1d_dir}/bin"

test1d_env="${test1d_dir}/pi_env.txt"
cat >"${test1d_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
env > "${test1d_env}"
printf '%s\n' "\$@" >> "${test1d_env}"
exit 0
MOCK_EOF
chmod +x "${test1d_dir}/bin/pi"
