...
#   each channel through Pi to observe system-reminder delivery.
#
# Usage:
#   bash .plankton/test/test_five_channels.sh           # automated tests
#   bash .plankton/test/test_five_channels.sh --runbook  # mitmproxy runbook
...
SETTINGS HELPER:
  Use swap_settings.sh to backup/restore your .pi/settings.json:
...
--------------------------------------------------------------------------------
  ...
  2. Write this to .pi/settings.json:
  ...
  3. Launch Pi with mitmproxy:

       HTTPS_PROXY=http://localhost:8080 pi --debug hooks
