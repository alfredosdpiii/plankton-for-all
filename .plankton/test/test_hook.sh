...
#
# Simulates the JSON input that Pi sends to PostToolUse hooks
# Useful for debugging hook behavior without running Pi
...
run_self_test() {
  ...
  mkdir -p "${fixture_project_dir}/.plankton/hooks"
  cp "${script_dir}/fixtures/config.json" \
    "${fixture_project_dir}/.plankton/config.json"
  # markdownlint-cli2 discovers config by walking up from the linted file
  cp "${script_dir}/fixtures/.markdownlint-cli2.jsonc" \
  ...
  test_temp_file() {
    ...
    set +e
    echo "${json_input}" | HOOK_SKIP_SUBPROCESS=1 PLANKTON_PROJECT_DIR="${fixture_project_dir}" "${hook_dir}/multi_linter.sh" >/dev/null 2>&1
    local actual_exit=$?
    set -e
  ...
  test_bash_command() {
    ...
    local pm_dir="$4"          # PLANKTON_PROJECT_DIR override (temp dir with config)
    ...
    output=$(echo "${json_input}" | PLANKTON_PROJECT_DIR="${pm_dir}" \
      "${hook_dir}/enforce_package_managers.sh" 2>/tmp/.pm_test_stderr_$$)
    local actual_exit=$?
...
echo "hello"' 0
  ...
  test_output_format() {
    ...
    set +e
    local output
    output=$(echo "${json_input}" | HOOK_SKIP_SUBPROCESS=1 PLANKTON_PROJECT_DIR="${fixture_project_dir}" "${hook_dir}/multi_linter.sh" 2>&1)
    set -e
...
RUN apt-get update' \
  ...
  test_model_selection() {
    ...
    set +e
    local output
    output=$(echo "${json_input}" | HOOK_SKIP_SUBPROCESS=1 HOOK_DEBUG_MODEL=1 PLANKTON_PROJECT_DIR="${fixture_project_dir}" "${hook_dir}/multi_linter.sh" 2>&1)
    set -e
...
def foo():
  ...
  mkdir -p "${ts_project_dir}/.plankton/hooks"
  cat >"${ts_project_dir}/.plankton/config.json" <<'TS_CFG_EOF'
{
  "languages": {
...
TS_CFG_EOF
  ...
  test_ts_file() {
    ...
    echo "${json_input}" | HOOK_SKIP_SUBPROCESS=1 \
      PLANKTON_PROJECT_DIR="${ts_project_dir}" \
      "${hook_dir}/multi_linter.sh" >/dev/null 2>&1
    local actual_exit=$?
    set -e
...
console.log(x);' 0
  ...
  return <img src="photo.jpg" />;
    ...
    mkdir -p "${ts_disabled_dir}/.plankton/hooks"
    cat >"${ts_disabled_dir}/.plankton/config.json" <<'TS_DIS_EOF'
{
  "languages": {
    "python": true, "shell": true, "yaml": true, "json": true,
    "toml": true, "dockerfile": true, "markdown": true,
...
TS_DIS_EOF
    ...
    echo "${ts_dis_json}" | HOOK_SKIP_SUBPROCESS=1 \
      PLANKTON_PROJECT_DIR="${ts_disabled_dir}" \
      "${hook_dir}/multi_linter.sh" >/dev/null 2>&1
    local ts_dis_exit=$?
    set -e
...
console.log(used);' \
    ...
    biome_protect_result=$(echo '{"tool_input":{"file_path":"biome.json"}}' \
      | PLANKTON_PROJECT_DIR="${ts_project_dir}" \
        "${hook_dir}/protect_linter_configs.sh" 2>/dev/null)
...
console.log("none used");' \
    ...
    mkdir -p "${d3_dir}/.plankton/hooks"
    cat >"${d3_dir}/.plankton/config.json" <<'D3_CFG_EOF'
{
  "languages": {
    "typescript": {
...
D3_SRC_EOF
  ...
  mkdir -p "${no_biome_dir}/.plankton/hooks"
  # Use js_runtime: "none" to force detect_biome() to find nothing
  cat >"${no_biome_dir}/.plankton/config.json" <<'NOBIOME_EOF'
{
  "languages": {
    "typescript": {
...
console.log(x);'
  ...
  echo "${no_biome_json}" | HOOK_SKIP_SUBPROCESS=1 \
    PLANKTON_PROJECT_DIR="${no_biome_dir}" \
    "${hook_dir}/multi_linter.sh" >/dev/null 2>&1
  no_biome_exit=$?
  set -e
  ...
  cat >"${pm_project_dir}/.plankton/config.json" <<'PM_CFG_EOF'
{
  "package_managers": {
    "python": "uv",
    "javascript": "bun",
...
PM_CFG_EOF
  ...
  cat >"${pm_warn_py_dir}/.plankton/config.json" <<'PM_WARN_PY_EOF'
{
  "package_managers": {
    "python": "uv:warn",
    "javascript": "bun",
...
PM_WARN_PY_EOF
  ...
  cat >"${pm_warn_js_dir}/.plankton/config.json" <<'PM_WARN_JS_EOF'
{
  "package_managers": {
    "python": "uv",
    "javascript": "bun:warn",
...
PM_WARN_JS_EOF
  ...
  cat >"${pm_off_py_dir}/.plankton/config.json" <<'PM_OFF_PY_EOF'
{
  "package_managers": {
    "python": false,
    "javascript": "bun",
...
PM_OFF_PY_EOF
  ...
  cat >"${pm_off_js_dir}/.plankton/config.json" <<'PM_OFF_JS_EOF'
{
  "package_managers": {
    "python": "uv",
    "javascript": false,
...
PM_OFF_JS_EOF
  ...
  skip_output=$(echo '{"tool_name": "Bash", "tool_input": {"command": "pip install requests"}}' \
    | HOOK_SKIP_PM=1 PLANKTON_PROJECT_DIR="${pm_project_dir}" \
      "${hook_dir}/enforce_package_managers.sh" 2>/dev/null || true)
  local skip_decision
  ...
  nojaq_output=$(echo '{"tool_name": "Bash", "tool_input": {"command": "pip install requests"}}' \
    | PATH="${mock_tools_dir}" PLANKTON_PROJECT_DIR="${pm_project_dir}" \
      "${hook_dir}/enforce_package_managers.sh" 2>/dev/null || echo '{"decision": "approve"}')
  set -e
  ...
  nouv_out=$(echo '{"tool_name":"Bash","tool_input":{"command":"pip install requests"}}' \
    | PATH="${mock_nouv}" PLANKTON_PROJECT_DIR="${pm_project_dir}" \
      "${hook_dir}/enforce_package_managers.sh" 2>/tmp/.pm_test_stderr_$$)
  ...
  nobun_out=$(echo '{"tool_name":"Bash","tool_input":{"command":"npm install lodash"}}' \
    | PATH="${mock_nobun}" PLANKTON_PROJECT_DIR="${pm_project_dir}" \
      "${hook_dir}/enforce_package_managers.sh" 2>/tmp/.pm_test_stderr_$$)
  ...
  dbg_out=$(echo '{"tool_name":"Bash","tool_input":{"command":"pip install requests"}}' \
    | HOOK_DEBUG_PM=1 PLANKTON_PROJECT_DIR="${pm_project_dir}" \
      "${hook_dir}/enforce_package_managers.sh" 2>/tmp/.pm_test_stderr_$$)
  ...
  test_stderr_json() {
    ...
    stderr_output=$(echo "${json_input}" | HOOK_SKIP_SUBPROCESS=1 \
      PLANKTON_PROJECT_DIR="${fixture_project_dir}" \
...
console.log("test");' >"${ts_feedback_file}"
  ...
  # hook_json stdout. Uses a mock pi binary that does nothing, so
  ...
  test_directive_codes() {
    ...
    # Mock pi: accepts any args, exits 0, does nothing to the file
    ...
    stdout_output=$(echo "${json_input}" \
      | PATH="${mock_dir}:${PATH}" \
        ...
        "${hook_dir}/multi_linter.sh" 2>/dev/null)
    local actual_exit=$?
    set -e
...
console.log("test");' \
  ...
  if grep -q 'RERUN_PHASE2_COUNT=' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q 'extract_violation_codes()' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q 'RERUN_PHASE2_RAW:-' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q '__init__\.py.*D100\|D100.*__init__\.py' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q 'exit_json()' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q 'RERUN_PHASE2_CODES=""' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q 'RERUN_PHASE2_CODES="${all_codes}"' < <(sed -n '/^rerun_phase2()/,/^[^ ]/p' "${hook_dir}/multi_linter.sh"); then
  ...
  if sed -n '/^extract_violation_codes()/,/^}/p' "${hook_dir}/multi_linter.sh" | grep -q 'RERUN_PHASE2_CODES'; then
  ...
  mkdir -p "${disabled_project}/.plankton/hooks"
  cat >"${disabled_project}/.plankton/config.json" <<'DIS_EOF'
...
DIS_EOF
  ...
  dis_stdout=$(echo '{"tool_input": {"file_path": "'"${disabled_file}"'"}}' \
    | PLANKTON_PROJECT_DIR="${disabled_project}" \
      "${hook_dir}/multi_linter.sh" 2>/dev/null)
  local dis_exit=$?
  set -e
  ...
  if grep -q '|| subprocess_exit=\$?' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q 'subprocess_exit=0' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -qE '^\s+subprocess_exit=\$\?\s*$' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q 'systemMessage.*2>/dev/null || printf' "${hook_dir}/multi_linter.sh"; then
  ...
  jaq_s_fallbacks=$(grep -c "jaq -s '\.')" "${hook_dir}/multi_linter.sh" || true)
  local jaq_s_with_fallback
  jaq_s_with_fallback=$(grep -c "jaq -s '\.') ||" "${hook_dir}/multi_linter.sh" || true)
  ...
  grep -q '|| json_violation=' "${hook_dir}/multi_linter.sh" && jaq_n_fallbacks=$((jaq_n_fallbacks + 1))
  grep -q '|| toml_violation=' "${hook_dir}/multi_linter.sh" && jaq_n_fallbacks=$((jaq_n_fallbacks + 1))
  ...
  if grep -q '|| semgrep_files=' "${hook_dir}/multi_linter.sh"; then
  ...
  if grep -q '|| hadolint_version=' "${hook_dir}/multi_linter.sh"; then
  ...
  local mock_pi_dir="${temp_dir}/mock_pi_bin"
  mkdir -p "${mock_pi_dir}"
  # Create mock 'pi' that always exits 1 (simulates subprocess crash)
  printf '#!/bin/bash\nexit 1\n' >"${mock_pi_dir}/pi"
  chmod +x "${mock_pi_dir}/pi"
  # Create mock 'timeout' that passes through to the command
  printf '#!/bin/bash\nshift; exec "$@"\n' >"${mock_pi_dir}/timeout"
  chmod +x "${mock_pi_dir}/timeout"
  ...
  if grep -q 'PROJECT_DIR.*.venv/bin' "${hook_dir}/multi_linter.sh"; then
  ...
  if command -v shellcheck >/dev/null 2>&1; then
    local sc_ok=true
    for sc_file in "${hook_dir}/multi_linter.sh" \
      ...
      "${script_dir}/test_subprocess_permissions.sh"; do
      if [[ -f "${sc_file}" ]]; then
...
if [[ -z "${file_path}" ]]; then
  ...
  echo ""
  echo "Exit codes:"
  echo "  0 - No issues or warnings only (not fed to Pi)"
  echo "  2 - Blocking errors found (fed to Pi via stderr)"
  exit 1
fi
...
# Construct JSON input like Pi does
json_input=$(
  cat <<EOF
{
  "tool_name": "Write",
...
script_dir="$(dirname "$(realpath "$0" || true)")"
set +e
echo "${json_input}" | "${hook_dir}/multi_linter.sh"
exit_code=$?
set -e
...
case ${exit_code} in
  0) echo "Status: OK (warnings only, not fed to Pi)" ;;
  2) echo "Status: BLOCKING (errors found, fed to Pi)" ;;
