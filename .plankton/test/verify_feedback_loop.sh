...
#   .plankton/test/verify_feedback_loop.sh
...
cp "${fixtures_dir}/config.json" "${fixture_project_dir}/.plankton/config.json"
...
run_check() {
  ...
  stderr_output=$(echo "${json_input}" \
    | HOOK_SKIP_SUBPROCESS=1 PLANKTON_PROJECT_DIR="${fixture_project_dir}" \
...
if ! command -v biome >/dev/null 2>&1; then
  ...
  ts_enabled=$(jaq -r '.languages.typescript.enabled // true' \
    "${fixture_project_dir}/.plankton/config.json" 2>/dev/null) || ts_enabled="false"
