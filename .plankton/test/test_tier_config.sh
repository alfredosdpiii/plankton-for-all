...
#
# Usage: bash .plankton/test/test_tier_config.sh
...
# Mock pi that logs args
t15a_args="${t15a_dir}/pi_args.txt"
cat >"${t15a_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15a_args}"
exit 0
MOCK_EOF
chmod +x "${t15a_dir}/bin/pi"
...
t15a_stderr=$(echo "${t15a_json}" \
  | PATH="${t15a_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15a_dir}/project" \
...
t15b_args="${t15b_dir}/pi_args.txt"
cat >"${t15b_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15b_args}"
exit 0
MOCK_EOF
chmod +x "${t15b_dir}/bin/pi"
...
t15b_stderr=$(echo "${t15b_json}" \
  | PATH="${t15b_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15b_dir}/project" \
...
t15b2_args="${t15b2_dir}/pi_args.txt"
cat >"${t15b2_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15b2_args}"
exit 0
MOCK_EOF
chmod +x "${t15b2_dir}/bin/pi"
...
t15b2_stderr=$(echo "${t15b2_json}" \
  | PATH="${t15b2_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15b2_dir}/project" \
...
t15c_args="${t15c_dir}/pi_args.txt"
cat >"${t15c_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15c_args}"
exit 0
MOCK_EOF
chmod +x "${t15c_dir}/bin/pi"
...
t15c_stderr=$(echo "${t15c_json}" \
  | PATH="${t15c_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15c_dir}/project" \
...
t15d_args="${t15d_dir}/pi_args.txt"
cat >"${t15d_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15d_args}"
exit 0
MOCK_EOF
chmod +x "${t15d_dir}/bin/pi"
...
t15d_stderr=$(echo "${t15d_json}" \
  | PATH="${t15d_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15d_dir}/project" \
...
t15e_args="${t15e_dir}/pi_args.txt"
cat >"${t15e_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15e_args}"
exit 0
MOCK_EOF
chmod +x "${t15e_dir}/bin/pi"
...
t15e_stderr=$(echo "${t15e_json}" \
  | PATH="${t15e_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15e_dir}/project" \
...
t15h_args="${t15h_dir}/pi_args.txt"
cat >"${t15h_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15h_args}"
exit 0
MOCK_EOF
chmod +x "${t15h_dir}/bin/pi"
...
echo "${t15h_json}" \
  | PATH="${t15h_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15h_dir}/project" \
...
t15j_args="${t15j_dir}/pi_args.txt"
cat >"${t15j_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15j_args}"
exit 0
MOCK_EOF
chmod +x "${t15j_dir}/bin/pi"
...
t15j_stderr=$(echo "${t15j_json}" \
  | PATH="${t15j_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15j_dir}/project" \
...
cat >"${t15k_dir}/bin/pi" <<'MOCK_EOF'
#!/bin/bash
exit 0
MOCK_EOF
chmod +x "${t15k_dir}/bin/pi"
...
t15k_stderr=$(echo "${t15k_json}" \
  | PATH="${t15k_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15k_dir}/project" \
...
t15m_args="${t15m_dir}/pi_args.txt"
cat >"${t15m_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t15m_args}"
exit 0
MOCK_EOF
chmod +x "${t15m_dir}/bin/pi"
...
t15m_stderr=$(echo "${t15m_json}" \
  | PATH="${t15m_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t15m_dir}/project" \
