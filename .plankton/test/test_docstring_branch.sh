...
#
# Uses mocked ruff + mocked pi for deterministic output.
#
# Usage: bash .plankton/test/test_docstring_branch.sh
...
# --- Helper: create mock pi that captures prompt ---
create_mock_pi() {
  local bin_dir="$1"
  local prompt_file="$2"
  local args_file="$3"
  mkdir -p "${bin_dir}"
  cat >"${bin_dir}/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${args_file}"
while [[ \$# -gt 0 ]]; do
  case "\$1" in
...
create_mock_pi "${t7a_dir}/bin" "${t7a_dir}/prompt.txt" "${t7a_dir}/args.txt"
...
echo "${t7a_json}" \
  | PATH="${t7a_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t7a_dir}/project" \
...
create_mock_pi "${t7b_dir}/bin" "${t7b_dir}/prompt.txt" "${t7b_dir}/args.txt"
...
echo "${t7b_json}" \
  | PATH="${t7b_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t7b_dir}/project" \
...
create_mock_pi "${t7d_dir}/bin" "${t7d_dir}/prompt.txt" "${t7d_dir}/args.txt"
...
echo "${t7d_json}" \
  | PATH="${t7d_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t7d_dir}/project" \
...
create_mock_pi "${t7g_dir}/bin" "${t7g_dir}/prompt.txt" "${t7g_dir}/args.txt"
...
echo "${t7g_json}" \
  | PATH="${t7g_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t7g_dir}/project" \
...
create_mock_pi "${t7e_dir}/bin" "${t7e_dir}/prompt.txt" "${t7e_dir}/args.txt"
...
echo "${t7e_json}" \
  | PATH="${t7e_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t7e_dir}/project" \
...
create_mock_pi "${t7h_dir}/bin" "${t7h_dir}/prompt.txt" "${t7h_dir}/args.txt"
...
echo "${t7h_json}" \
  | PATH="${t7h_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t7h_dir}/project" \
...
create_mock_pi "${t7i_dir}/bin" "${t7i_dir}/prompt.txt" "${t7i_dir}/args.txt"
...
echo "${t7i_json}" \
  | PATH="${t7i_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t7i_dir}/project" \
