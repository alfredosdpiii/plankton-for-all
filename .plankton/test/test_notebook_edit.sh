...
# - Protected .pi/ paths still block via notebook_path
#
# Usage: bash .plankton/test/test_notebook_edit.sh
...
t8b_out=$(run_protect '{"tool_input":{"notebook_path":"/home/user/.plankton/hooks/bad.sh"}}' "${tmp_dir}/t8b")
t8b_decision=$(echo "${t8b_out}" | jaq -r '.decision' 2>/dev/null)

assert "t8b_notebook_path" \
  "[[ '${t8b_decision}' == 'block' ]]" \
  "notebook_path fallback blocks .plankton/hooks/ path" \
...
# ============================================================================
# Test 8e: .plankton/hooks/ blocks through notebook_path
# ============================================================================
printf "\n--- test8e: .plankton/hooks/ blocks through notebook_path ---\n"

t8e_out=$(run_protect '{"tool_input":{"notebook_path":".plankton/hooks/multi_linter.sh"}}' "${tmp_dir}/t8e")
t8e_decision=$(echo "${t8e_out}" | jaq -r '.decision' 2>/dev/null)

assert "t8e_block_hooks" \
  "[[ '${t8e_decision}' == 'block' ]]" \
  ".plankton/hooks/ blocked via notebook_path" \
  ".plankton/hooks/ NOT blocked via notebook_path (decision=${t8e_decision})"

# ============================================================================
# Test 8f: .plankton/subprocess-settings.json blocks through notebook_path
# ============================================================================
printf "\n--- test8f: subprocess-settings blocks through notebook_path ---\n"

t8f_out=$(run_protect '{"tool_input":{"notebook_path":".plankton/subprocess-settings.json"}}' "${tmp_dir}/t8f")
