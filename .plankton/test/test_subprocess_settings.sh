...
#
# Verifies that:
# - Default settings path is project-local .plankton/subprocess-settings.json
...
#
# Usage: bash .plankton/test/test_subprocess_settings.sh
...
t2a_args="${t2a_dir}/pi_args.txt"
cat >"${t2a_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t2a_args}"
exit 0
MOCK_EOF
chmod +x "${t2a_dir}/bin/pi"

setup_project_dir "${t2a_dir}/project" "${TIER_CONFIG}"

# Create the project-local settings file (as the hook would expect)
mkdir -p "${t2a_dir}/project/.pi"
cat >"${t2a_dir}/project/.plankton/subprocess-settings.json" <<'SETTINGS_EOF'
{
...
echo "${t2a_json}" \
  | PATH="${t2a_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t2a_dir}/project" \
...
t2b_args="${t2b_dir}/pi_args.txt"
cat >"${t2b_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t2b_args}"
exit 0
MOCK_EOF
chmod +x "${t2b_dir}/bin/pi"
...
echo "${t2b_json}" \
  | PATH="${t2b_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t2b_dir}/project" \
...
settings_file="${project_dir}/.plankton/subprocess-settings.json"
...
t2e_dir="${tmp_dir}/t2e"
mkdir -p "${t2e_dir}/bin"
t2e_args="${t2e_dir}/pi_args.txt"
cat >"${t2e_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t2e_args}"
exit 0
MOCK_EOF
chmod +x "${t2e_dir}/bin/pi"
...
echo "${t2e_json}" \
  | PATH="${t2e_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t2e_dir}/project" \
...
t2f_args="${t2f_dir}/pi_args.txt"
cat >"${t2f_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
printf '%s\n' "\$@" > "${t2f_args}"
exit 0
MOCK_EOF
chmod +x "${t2f_dir}/bin/pi"

setup_project_dir "${t2f_dir}/project" "${TIER_CONFIG}"

# Create the project-local settings file
mkdir -p "${t2f_dir}/project/.pi"
cat >"${t2f_dir}/project/.plankton/subprocess-settings.json" <<'SETTINGS_EOF'
{
  "$schema": "https://json.schemastore.org/pi-code-settings.json",
  "disableAllHooks": true,
  "skipDangerousModePermissionPrompt": true
...
echo "${t2f_json}" \
  | PATH="${t2f_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${t2f_dir}/project" \
