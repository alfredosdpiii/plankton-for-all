...
#
# Usage: bash .plankton/test/test_hook_timing.sh
...
# --- Helper: create mock pi that exits 0 without modifying file ---
create_mock_pi() {
  local bin_dir="$1"
...
run_hook() {
  ...
  create_mock_pi "${test_dir}/bin" "${test_dir}/args.txt"
  printf 'import os\ndef hello():\n    pass\n' >"${test_dir}/test_file.py"

  echo "${json_payload}" \
    | PATH="${test_dir}/bin:${mock_bin}:${PATH}" \
      PLANKTON_PROJECT_DIR="${test_dir}/project" \
...
create_mock_pi "${t1b_dir}/bin" "${t1b_dir}/args.txt"
printf 'import os\ndef hello():\n    pass\n' >"${t1b_dir}/test_file.py"

echo "${t1b_json}" \
  | PATH="${t1b_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t1b_dir}/project" \
...
create_mock_pi "${t1c_dir}/bin" "${t1c_dir}/args.txt"
printf 'import os\ndef hello():\n    pass\n' >"${t1c_dir}/test_file.py"

t1c_exit=0
echo "${t1c_json}" \
  | PATH="${t1c_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t1c_dir}/project" \
...
# ============================================================================
# t3c: phase=delegate_end with changed=no (mock pi does not modify file)
# ============================================================================
...
assert "t3c_changed_no" \
  ...
  "delegate_end has changed=no (mock pi did not modify file)" \
...
create_mock_pi "${t5a_dir}/bin" "${t5a_dir}/args.txt"
printf 'import os\ndef hello():\n    pass\n' >"${t5a_dir}/test_file.py"

echo "${t5a_json}" \
  | PATH="${t5a_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t5a_dir}/project" \
...
create_mock_pi "${t6a_dir}/bin" "${t6a_dir}/args.txt"
printf 'import os\ndef hello():\n    pass\n' >"${t6a_dir}/test_file.py"

echo "${t6a_json}" \
  | PATH="${t6a_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t6a_dir}/project" \
...
create_mock_pi "${t6b_dir}/bin" "${t6b_dir}/args.txt"
printf 'import os\ndef hello():\n    pass\n' >"${t6b_dir}/test_file.py"

echo "${t6b_json}" \
  | PATH="${t6b_dir}/bin:${mock_bin}:${PATH}" \
    PLANKTON_PROJECT_DIR="${t6b_dir}/project" \
