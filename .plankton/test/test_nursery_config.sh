...
#
# Usage: bash .plankton/test/test_nursery_config.sh
...
BIOME_EOF
  ...
  cat >"${pd}/.plankton/config.json" <<CONFIG_EOF
{
  "languages": {
    "typescript": {
      "enabled": true,
...
echo '{"tool_input":{"file_path":"'"${test_n1_ts}"'"}}' \
  | PLANKTON_PROJECT_DIR="${test_n1_dir}" \
...
echo '{"tool_input":{"file_path":"'"${test_n2_ts}"'"}}' \
  | PLANKTON_PROJECT_DIR="${test_n2_dir}" \
...
echo '{"tool_input":{"file_path":"'"${test_n3_ts}"'"}}' \
  | PLANKTON_PROJECT_DIR="${test_n3_dir}" \
