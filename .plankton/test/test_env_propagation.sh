...
#       shell environment ARE inherited by the Phase 3 subprocess (pi -p)
# - 2b: CLAUDECODE is NOT inherited (env -u CLAUDECODE strips it)
#        [complements test1d in test_subprocess_permissions.sh]
#
# Background: Pi injects `env` block values from --settings into its own
...
#
# Usage: bash .plankton/test/test_env_propagation.sh
...
test2a_env="${test2a_dir}/pi_env.txt"
cat >"${test2a_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
env > "${test2a_env}"
printf '%s\n' "\$@" >> "${test2a_env}"
exit 0
MOCK_EOF
chmod +x "${test2a_dir}/bin/pi"
...
echo "${test2a_json}" \
  | PATH="${test2a_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test2a_dir}/project" \
...
test2b_env="${test2b_dir}/pi_env.txt"
cat >"${test2b_dir}/bin/pi" <<MOCK_EOF
#!/bin/bash
env > "${test2b_env}"
printf '%s\n' "\$@" >> "${test2b_env}"
exit 0
MOCK_EOF
chmod +x "${test2b_dir}/bin/pi"
