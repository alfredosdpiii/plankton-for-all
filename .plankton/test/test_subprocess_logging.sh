...
#
# Usage: bash .plankton/test/test_subprocess_logging.sh
...
# Mock pi that writes diagnostic output to stderr
cat >"${test3a_dir}/bin/pi" <<'MOCK_EOF'
#!/bin/bash
echo "SUBPROCESS_STDERR_MARKER" >&2
exit 0
MOCK_EOF
chmod +x "${test3a_dir}/bin/pi"
...
test3a_stderr=$(echo "${test3a_json}" \
  | PATH="${test3a_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test3a_dir}/project" \
...
# 3b-modified: mock pi that modifies the file
test3b_dir="${tmp_dir}/test3b"
mkdir -p "${test3b_dir}/bin"

cat >"${test3b_dir}/bin/pi" <<'MOCK_EOF'
#!/bin/bash
...
test3b_stderr=$(echo "${test3b_json}" \
  | PATH="${test3b_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test3b_dir}/project" \
...
# 3b-unmodified: mock pi that does nothing
test3b2_dir="${tmp_dir}/test3b2"
mkdir -p "${test3b2_dir}/bin"

cat >"${test3b2_dir}/bin/pi" <<'MOCK_EOF'
#!/bin/bash
exit 0
MOCK_EOF
chmod +x "${test3b2_dir}/bin/pi"
...
test3b2_stderr=$(echo "${test3b2_json}" \
  | PATH="${test3b2_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test3b2_dir}/project" \
...
cat >"${test3c_dir}/bin/pi" <<'MOCK_EOF'
#!/bin/bash
exit 0
MOCK_EOF
chmod +x "${test3c_dir}/bin/pi"
...
test3c_stderr=$(echo "${test3c_json}" \
  | PATH="${test3c_dir}/bin:${PATH}" \
    PLANKTON_PROJECT_DIR="${test3c_dir}/project" \
