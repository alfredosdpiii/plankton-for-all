...
#
# Requires: pi binary on PATH, ANTHROPIC_API_KEY set, network access
#
# Usage: bash .plankton/test/test_empirical_p3_p4.sh
...
if ! command -v pi >/dev/null 2>&1; then
  printf "SKIP: pi binary not on PATH\n"
  exit 0
fi
...
cat >"${p4_settings}" <<'EOF'
{
  ...
  "skipDangerousModePermissionPrompt": true,
  "env": {
    "ANTHROPIC_MODEL": "pi-opus-4-20250514"
  }
}
EOF
