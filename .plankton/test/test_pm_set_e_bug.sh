...
#
# Usage: bash .plankton/test/test_pm_set_e_bug.sh
...
t5d_dir="${tmp_dir}/t5d"
mkdir -p "${t5d_dir}" # NO .plankton/config.json

t5d_out="${tmp_dir}/t5d_stdout.txt"
t5d_exit=0
echo '{"tool_input":{"command":"pip install requests"}}' \
  | PLANKTON_PROJECT_DIR="${t5d_dir}" \
