...
# - Blocks relative .pi/settings.json paths (BUG-9a)
# - Blocks relative .pi/settings.local.json paths (BUG-9a)
# - Blocks relative .plankton/subprocess-settings.json paths (BUG-9)
# - Blocks absolute-style .pi/* paths (existing)
...
# - Existing .plankton/hooks/* protection still works
#
# Verifies stop_config_guardian.sh (Stop hook):
# - Detects modified .plankton/subprocess-settings.json via git diff
# - Approves unmodified repos
# - Bypasses check when stop_hook_active=true
#
# Usage: bash .plankton/test/test_subprocess_protection.sh
...
run_protect() {
  ...
  mkdir -p "${project_dir}/.plankton/hooks"
  if [[ -n "${config_content}" ]]; then
    printf '%s\n' "${config_content}" >"${project_dir}/.plankton/config.json"
  fi
...
# ============================================================================
# Test 9a: relative .pi/settings.json blocks
# ============================================================================
printf "\n--- test9a: relative .pi/settings.json blocks ---\n"

t9a_out=$(run_protect ".pi/settings.json" "${tmp_dir}/t9a")
t9a_decision=$(echo "${t9a_out}" | jaq -r '.decision' 2>/dev/null)

assert "t9a_block" \
  "[[ '${t9a_decision}' == 'block' ]]" \
  "relative .pi/settings.json blocked" \
  "relative .pi/settings.json NOT blocked (decision=${t9a_decision})"

# ============================================================================
# Test 9b: absolute-style .pi/settings.json blocks
# ============================================================================
printf "\n--- test9b: absolute-style .pi/settings.json blocks ---\n"

t9b_out=$(run_protect "/home/user/project/.pi/settings.json" "${tmp_dir}/t9b")
t9b_decision=$(echo "${t9b_out}" | jaq -r '.decision' 2>/dev/null)

assert "t9b_block" \
  "[[ '${t9b_decision}' == 'block' ]]" \
  "absolute .pi/settings.json blocked" \
  "absolute .pi/settings.json NOT blocked (decision=${t9b_decision})"

# ============================================================================
# Test 9c: relative .pi/settings.local.json blocks
# ============================================================================
printf "\n--- test9c: relative .pi/settings.local.json blocks ---\n"

t9c_out=$(run_protect ".pi/settings.local.json" "${tmp_dir}/t9c")
t9c_decision=$(echo "${t9c_out}" | jaq -r '.decision' 2>/dev/null)

assert "t9c_block" \
  "[[ '${t9c_decision}' == 'block' ]]" \
  "relative .pi/settings.local.json blocked" \
  "relative .pi/settings.local.json NOT blocked (decision=${t9c_decision})"

# ============================================================================
# Test 9d: relative .plankton/subprocess-settings.json blocks
# ============================================================================
printf "\n--- test9d: relative .plankton/subprocess-settings.json blocks ---\n"

t9d_out=$(run_protect ".plankton/subprocess-settings.json" "${tmp_dir}/t9d")
t9d_decision=$(echo "${t9d_out}" | jaq -r '.decision' 2>/dev/null)

assert "t9d_block" \
  "[[ '${t9d_decision}' == 'block' ]]" \
  "relative .plankton/subprocess-settings.json blocked" \
  "relative .plankton/subprocess-settings.json NOT blocked (decision=${t9d_decision})"

# ============================================================================
# Test 9e: absolute-style .plankton/subprocess-settings.json blocks
# ============================================================================
printf "\n--- test9e: absolute .plankton/subprocess-settings.json blocks ---\n"

t9e_out=$(run_protect "/home/user/project/.plankton/subprocess-settings.json" "${tmp_dir}/t9e")
t9e_decision=$(echo "${t9e_out}" | jaq -r '.decision' 2>/dev/null)

assert "t9e_block" \
  "[[ '${t9e_decision}' == 'block' ]]" \
  "absolute .plankton/subprocess-settings.json blocked" \
  "absolute .plankton/subprocess-settings.json NOT blocked (decision=${t9e_decision})"
...
EXTERNAL_CONFIG='{"subprocess":{"settings_file":"/opt/pi/custom-settings.json"}}'

t9f_out=$(run_protect "/opt/pi/custom-settings.json" "${tmp_dir}/t9f" "${EXTERNAL_CONFIG}")
...
# ============================================================================
# Test 9i: existing .plankton/hooks/* protection still works
# ============================================================================
printf "\n--- test9i: .plankton/hooks/* protection works ---\n"

t9i_out=$(run_protect "/home/user/project/.plankton/hooks/multi_linter.sh" "${tmp_dir}/t9i")
t9i_decision=$(echo "${t9i_out}" | jaq -r '.decision' 2>/dev/null)

assert "t9i_block" \
  "[[ '${t9i_decision}' == 'block' ]]" \
  ".plankton/hooks/* blocked" \
  ".plankton/hooks/* NOT blocked (decision=${t9i_decision})"

# ============================================================================
# Test 9i2: .plankton/test/* is explicitly allowed
# ============================================================================
printf "\n--- test9i2: .plankton/test/* is allowed ---\n"

t9i2_out=$(run_protect "/home/user/project/.plankton/test/test_docstring_branch.sh" "${tmp_dir}/t9i2")
t9i2_decision=$(echo "${t9i2_out}" | jaq -r '.decision' 2>/dev/null)

assert "t9i2_approve" \
  "[[ '${t9i2_decision}' == 'approve' ]]" \
  ".plankton/test/* approved" \
  ".plankton/test/* NOT approved (decision=${t9i2_decision})"

# ============================================================================
# Test 9i3: .plankton/test fixtures bypass basename-based linter config blocks
# ============================================================================
printf "\n--- test9i3: .plankton/test fixtures are allowed ---\n"

t9i3_out=$(run_protect "/home/user/project/.plankton/test/fixtures/.markdownlint.jsonc" "${tmp_dir}/t9i3")
t9i3_decision=$(echo "${t9i3_out}" | jaq -r '.decision' 2>/dev/null)

assert "t9i3_approve" \
  "[[ '${t9i3_decision}' == 'approve' ]]" \
  ".plankton/test fixture config approved" \
  ".plankton/test fixture config NOT approved (decision=${t9i3_decision})"
...
setup_git_repo() {
  local repo_dir="$1"
  mkdir -p "${repo_dir}/.plankton/hooks"
  (cd "${repo_dir}" \
    && git init -q \
    && git config user.name "test" \
    && git config user.email "test@test" \
    && echo '{}' >".plankton/config.json" \
    && echo '{}' >".plankton/subprocess-settings.json" \
    && git add -A \
    && git commit -q -m "init")
}
...
echo '{"disableAllHooks": true}' >"${t9k_dir}/.plankton/subprocess-settings.json"

t9k_out=$(cd "${t9k_dir}" && echo '{"stop_hook_active": false}' \
  | PLANKTON_PROJECT_DIR="${t9k_dir}" \
